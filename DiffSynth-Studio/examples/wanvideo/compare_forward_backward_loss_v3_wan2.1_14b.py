...
v3 Changes:
- Input: JSON dataset with video_path_forward/backward format
- Output: Structured JSON with step_losses and summary_metrics
- FPS resampling using torchvision.io
- Bucket-based resolution (short edge resize + center crop)
- Sliding window for long videos with accumulated loss
...
# =============================================================================
# Model Constants for Wan2.1
# =============================================================================
MODEL_TARGET_FPS = 16
MODEL_FRAME_LIMIT = 33  # 4n+1 format, typical max frames per window
MODEL_FRAME_LIMIT_LOW_VRAM = 17  # Reduced frames for low VRAM mode (4*12+1)
TIME_DIVISION_FACTOR = 4
TIME_DIVISION_REMAINDER = 1
HEIGHT_DIVISION_FACTOR = 16
WIDTH_DIVISION_FACTOR = 16

# Bucket resolutions supported by Wan2.1 (height, width)
# These are common resolutions that maintain aspect ratio properly
BUCKET_RESOLUTIONS = [
...
# Lower resolution buckets for low VRAM mode (24GB)
BUCKET_RESOLUTIONS_LOW_VRAM = [
    ...
    (480, 832),   # ~16:9 landscape (max for low vram)
    (832, 480),   # ~16:9 portrait
]
...
class LossComputer:
    ...
    def compute_loss_with_fixed_noise(self, inputs, noise, timestep, skip_latent_frames: int = 0):
        ...
        # Ensure DiT model is loaded to GPU (supports VRAM-managed CPU offloading)
...
def get_video_info(video_path: str) -> Tuple[float, int, int, int]:
    ...
    Returns: (fps, width, height, total_frames)
    """
    cmd = [
...
def select_bucket_resolution(orig_width: int, orig_height: int, low_vram: bool = False) -> Tuple[int, int]:
    ...
    Strategy: Choose bucket that best matches aspect ratio while minimizing upscaling.
    """
    ...
    for bucket_h, bucket_w in bucket_list:
        ...
        # Prefer buckets that don't require upscaling
        scale_factor = max(bucket_w / orig_width, bucket_h / orig_height)
        upscale_penalty = max(0, scale_factor - 1) * 2  # Penalize upscaling
...
def load_video_with_ffmpeg(
    ...
    2. Resize short edge to match target while maintaining aspect ratio
    3. Center crop to exact target resolution
    """
    # Create temp directory for frames
    with tempfile.TemporaryDirectory() as tmpdir:
        # Build ffmpeg filter for:
        # 1. FPS conversion
        # 2. Scale (short edge fit)
        # 3. Center crop

        # Determine scale filter: scale to fit short edge
        # If target is landscape (w > h), scale height to target_height
        # If target is portrait (h > w), scale width to target_width
...
def slice_video_into_windows(
    ...
    while start < len(frames):
        ...
        # If last window is shorter, prepend frames from previous segment
        ...
        # Adjust window size to meet 4n+1 requirement
        adjusted_size = adjust_frame_count(len(window))
        window = window[:adjusted_size]

        # Adjust num_borrowed if window was truncated from the end
        # (borrowed frames are at the beginning, so they're not affected)

        if len(window) >= 5:  # Minimum viable window size
...
def reverse_video(video_frames: List[Image.Image]) -> List[Image.Image]:
    """Reverse video (reverse the frame sequence)"""
...
def compute_video_loss(
    ...
    Returns: (average_loss, step_losses_dict)
    """
...
def main():
    parser = argparse.ArgumentParser(description="Compare forward and backward video denoising loss (v3)")

    # Dataset arguments
    parser.add_argument("--dataset_path", type=str, required=True,
                        help="Path to dataset metadata JSON file")
    parser.add_argument("--dataset_base_path", type=str, default="",
                        help="Base path to prepend to video paths in dataset (if relative)")
    parser.add_argument("--max_samples", type=int, default=None,
                        help="Maximum number of samples to process")

    # Model arguments
    parser.add_argument("--model_id_with_origin_paths", type=str,
                        default="Wan-AI/Wan2.1-T2V-14B:diffusion_pytorch_model*.safetensors,Wan-AI/Wan2.1-T2V-14B:models_t5_umt5-xxl-enc-bf16.pth,Wan-AI/Wan2.1-T2V-14B:Wan2.1_VAE.pth",
                        help="Model paths")
    ...
    parser.add_argument("--output_path", type=str, default="./result_output.json", help="Output path")
    ...
    parser.add_argument("--low_vram", action="store_true",
                        help="Enable aggressive low VRAM mode (disk offload) for 24GB GPUs")

    # Multi-GPU parallel processing arguments
    ...
    if args.low_vram:
        # Reduce window_size to save VRAM
        if args.window_size == MODEL_FRAME_LIMIT:  # User did not customize
    ...
    vram_config = None
    if args.low_vram:
        # Aggressive low VRAM mode: disk offload, suitable for 24GB GPUs
        vram_config = {
            "offload_dtype": "disk",
            "offload_device": "disk",
        ...
        # Automatically compute VRAM limit
    ...
    # Prepare tokenizer config (tokenizer requires separate configuration)
    # Extract base model name from model_id to configure tokenizer
    base_model_id = args.model_id_with_origin_paths.split(",")[0].split(":")[0]
    tokenizer_config = ModelConfig(model_id=base_model_id, origin_file_pattern="google/umt5-xxl/")
    ...
    if start_idx > 0 or end_idx < total_len:
        ...
        print(f"[Info] Processing subset: indices {start_idx} to {end_idx}")
    ...
    # Record loss difference for each timestep
    ...
    with torch.no_grad():
        for idx in tqdm(range(len(dataset)), desc="Processing videos"):
            ...
            try:
                ...
                # Compute forward and backward losses with matching random noise.
    ...
    print(f"  Forward wins: {forward_wins} ({forward_wins/total_valid*100:.2f}%)" if total_valid else "  Forward wins: 0")
    print(f"  Backward wins: {backward_wins} ({backward_wins/total_valid*100:.2f}%)" if total_valid else "  Backward wins: 0")
    print(f"  Ties: {ties}")
    print(f"  PPE (Probability of Preferring Error): {ppe:.4f}")
    print("="*50)
