...
For Wan2.2-T2V-A14B model

v3-Wan2.2-A14B Changes:
- Optimized for Wan2.2-T2V-A14B model (dual DiT architecture: high_noise_model + low_noise_model)
- Uses Wan2.1_VAE (same as 14B model)
...
- Handles dual DiT model switching at switch_DiT_boundary (default 0.5)
"""
import torch
import os
import argparse
...
# =============================================================================
# Model Constants for Wan2.2-T2V-A14B
# =============================================================================
MODEL_TARGET_FPS = 16
MODEL_FRAME_LIMIT = 33  # 4n+1 format, typical max frames per window
MODEL_FRAME_LIMIT_LOW_VRAM = 17  # Reduced frames for low VRAM mode (4*4+1)
TIME_DIVISION_FACTOR = 4
TIME_DIVISION_REMAINDER = 1
HEIGHT_DIVISION_FACTOR = 16
WIDTH_DIVISION_FACTOR = 16

# Default switch boundary for dual DiT models (high_noise -> low_noise)
# timestep < switch_DiT_boundary * 1000 will use low_noise_model (dit2)
DEFAULT_SWITCH_DIT_BOUNDARY = 0.5
...
class LossComputer:
    def __init__(self, pipe, use_gradient_checkpointing=False, switch_dit_boundary=DEFAULT_SWITCH_DIT_BOUNDARY):
        self.pipe = pipe
    ...
    def get_dit_model_for_timestep(self, timestep_value):
        """Select the correct DiT model based on timestep (high_noise or low_noise)"""
        # Wan2.2-A14B uses dual DiT architecture:
        # - timestep >= switch_dit_boundary * 1000: use dit (high_noise_model)
        # - timestep < switch_dit_boundary * 1000: use dit2 (low_noise_model)
        if timestep_value < self.switch_dit_boundary * 1000 and self.pipe.dit2 is not None:
            return self.pipe.dit2, "dit2"
        else:
            return self.pipe.dit, "dit"
    ...
    def compute_loss_with_fixed_noise(self, inputs, noise, timestep, skip_latent_frames: int = 0):
        ...
        # Select the correct DiT model
        timestep_value = timestep.item()
        dit_model, dit_name = self.get_dit_model_for_timestep(timestep_value)

        # Ensure the corresponding DiT model is loaded to GPU
        if dit_name == "dit2":
            self.pipe.load_models_to_device(self.pipe.in_iteration_models_2)
        else:
            self.pipe.load_models_to_device(self.pipe.in_iteration_models)

        # Build model dictionary
        models = {name: getattr(self.pipe, name) for name in self.pipe.in_iteration_models}
        models["dit"] = dit_model  # Replace with the correct DiT model
...
def main():
    parser = argparse.ArgumentParser(description="Compare forward and backward video denoising loss for Wan2.2-T2V-A14B")
    ...
    parser.add_argument("--model_id_with_origin_paths", type=str,
                        default="Wan-AI/Wan2.2-T2V-A14B:high_noise_model/diffusion_pytorch_model*.safetensors,Wan-AI/Wan2.2-T2V-A14B:low_noise_model/diffusion_pytorch_model*.safetensors,Wan-AI/Wan2.2-T2V-A14B:models_t5_umt5-xxl-enc-bf16.pth,Wan-AI/Wan2.2-T2V-A14B:Wan2.1_VAE.pth",
                        help="Model paths for Wan2.2-T2V-A14B")
    ...
    parser.add_argument("--output_path", type=str, default="./result_output_wan2.2_a14b.json", help="Output path")
    ...
    parser.add_argument("--switch_dit_boundary", type=float, default=DEFAULT_SWITCH_DIT_BOUNDARY,
                        help="Boundary for switching between high_noise and low_noise DiT models (default: 0.5)")
    ...
    # Load model
    print("Loading Wan2.2-T2V-A14B model (dual DiT architecture)...")
    ...
    # Check if dual DiT models were loaded successfully
    if pipe.dit2 is not None:
        print(f"[Info] Dual DiT models loaded: dit (high_noise) and dit2 (low_noise)")
        print(f"[Info] Switch boundary: {args.switch_dit_boundary} (timestep < {args.switch_dit_boundary * 1000} uses low_noise model)")
    else:
        print(f"[Warning] dit2 not loaded, using single DiT model")
    ...
    loss_computer = LossComputer(pipe, args.use_gradient_checkpointing, args.switch_dit_boundary)
    ...
    # Mark which DiT model is used for each timestep
    for t in sampled_timesteps:
        model_name = "low_noise (dit2)" if t < args.switch_dit_boundary * 1000 else "high_noise (dit)"
        print(f"  Timestep {t}: {model_name}")
    ...
    with torch.no_grad():
        for idx in tqdm(range(len(dataset)), desc="Processing videos"):
            ...
            try:
                ...
                if pipe.dit2 is not None:
                    model_params_dit2 = sum(p.numel() for p in pipe.dit2.parameters())
                else:
                    model_params_dit2 = 0
                ...
                output = {
                    "model_info": {
                        ...
                        "parameters_dit2": model_params_dit2,
                        "architecture": "dual_dit",
